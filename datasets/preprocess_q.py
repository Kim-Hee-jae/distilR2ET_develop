...
def process(positions):
    ...
    feet_r_x = ((positions[1:, fid_r, 0] - positions[:-1, fid_r, 0]) / char_height * 180) ** 2
    feet_r_y = ((positions[1:, fid_r, 1] - positions[:-1, fid_r, 1]) / char_height * 180) ** 2
    feet_r_z = ((positions[1:, fid_r, 2] - positions[:-1, fid_r, 2]) / char_height * 180) ** 2
    feet_r_h = positions[:-1, fid_r, 1] / char_height * 180
    feet_r = (
        ((feet_r_x + feet_r_y + feet_r_z) < velfactor) & (feet_r_h < heightfactor)
        # (feet_r_h < heightfactor)
    ).astype(float)

    # if np.sum(positions[:-1, fid_l, 1] < -0) > 0:
    #     print(floor_height)
    #     print(feet_l)
    #     print(feet_r)
    # if floor_height > 50:
...
data_paths = ["../service/jobs/"]
save_path = "../service/jobs/"
joints_list = [
    "Spine",
    "Spine1",
    "Spine2",
