...
def extract_data(fbx_path, subject_name, save_path):
    ...
    for obj in scene.objects:
        if obj.type == 'MESH' and not obj.name == 'Cube':
            ...
            # 임시 코드 1: Main Mesh 찾기 <-- 나중에 더 정교화 해야함
            if len(rest_verts_lst) < 1000:
                continue
            main_obj = obj
            print('Main mesh found:', len(rest_verts_lst), len(rest_faces_lst))
    ...
    for obj in scene.objects:
        if obj.type == 'MESH' and not obj.name == 'Cube':
            # 임시 코드 2: Main Mesh 찾기 <-- 나중에 더 정교화 해야함
            if obj is not main_obj:
                continue
            print('Main mesh found again')
    ...
    print("simplified joints:", bvh_data.simplified_joint_names)
    print("num verts:", np_rest_verts.shape[0], "num vgrps:", len(vgrp_label))
    print("body_vid count:", len(body_vid_lst), "arm_vid count:", len(arm_vid_lst))
    unique, counts = np.unique(vertex_part, return_counts=True)
    print("vertex_part histogram:", dict(zip(unique.tolist(), counts.tolist())))
...
if __name__ == '__main__':
    root_path = sys.argv[-1] # 이거 바꿔야함
    fbx_name_lst = [fbx_name for fbx_name in listdir(root_path) if fbx_name.endswith(".fbx")]

    for fbx_name in tqdm(fbx_name_lst):
        subject_name = fbx_name.split(".")[0]
        fbx_path = join(root_path, fbx_name)
        save_path = sys.argv[-1] # 이것도 바꿔야함
