...
class Feeder(Dataset):
    def __init__(self, data_path, stats_path, shape_path, gt_path, max_length, data_config_path, phase): # source_chars, target_char
        ...
        if "train" in phase:
            self.source_chars = data_config['seen_char']
            self.source_motions = data_config['seen_motion']
        elif "test_ss" in phase: 
            self.source_chars = data_config['seen_char']
            self.source_motions = data_config['seen_motion']
        elif "test_us" in phase:
            self.source_chars = data_config['unseen_char']
            self.source_motions = data_config['seen_motion']
        elif "test_su" in phase:
            self.source_chars = data_config['seen_char']
            self.source_motions = data_config['unseen_motion']
        elif "test_uu" in phase:
            self.source_chars = data_config['unseen_char']
            self.source_motions = data_config['unseen_motion']
        else:
            self.source_chars = data_config['seen_char']
        ...
        self.target_char = data_config['target_char']
        ...
        # source
        for shape_name in self.source_chars:
            fbx_file = np.load(join(shape_path, shape_name+'.npz'))
            ...
            self.shape_dic[shape_name] = shape_vector

            shape_lst.append(shape_vector[:, :])

        # target
        fbx_file = np.load(join(dirname(gt_path), self.target_char+'.npz'))
        full_width = fbx_file['full_width'].astype(np.single)
        joint_shape = fbx_file['joint_shape'].astype(np.single)

        shape_vector = np.divide(joint_shape, full_width[None, :])
        self.shape_dic[self.target_char] = shape_vector
        ...
        # -----------------------------------------------------------------------------------------------
        if "tmp" in phase:
            self._gt_path = "./service/jobs/tmp"
        else:
             self._gt_path = self.gt_path
        self.load_data()
    ...
    def load_data(self):
        ...
        for folder_name in folders:
            ...
            # ]
            files = self.source_motions
            for cfile in files:
                file_name = basename(cfile[:-4])
                # Real joint positions
                positions = np.load(
                    join(self.data_path, folder_name, file_name + "_skel.npy")
                )
                positions_B = np.load(
                    join(dirname(self.gt_path), self.target_char + "_skel.npy")
                )
                ...
                sequence_B = np.load(
                    join(dirname(self.gt_path), self.target_char + "_seq.npy")
                )
                ...
                sequence_B = np.reshape(sequence_B[:, :-8], [sequence_B.shape[0], -1, 3])
                positions_B[:, 0, :] = sequence_B[:, 0, :]  # root joint

                all_local.append(sequence)
                all_global.append(offset)
                all_skel.append(positions)
                all_skel_B.append(positions_B)
                ...
                # ground truth quat (#frames, #joints, 4) <- change at tmp phase
                
                quat = np.load(
                    join(self._gt_path, folder_name, file_name + ".npz")
                )
                all_quatB.append(quat['quatB_rt'][0])
                all_dqs.append(quat['delta_qs'][0])
                all_dqg.append(quat['delta_qg'][0])
                

        # Joint positions before processed
        train_skel = all_skel  # N T J 3
        train_skel_B = all_skel_B
        ...
        self.all_foot_contact = all_foot_contact

        self.all_quatB = all_quatB
        self.all_dqs = all_dqs
        self.all_dqg = all_dqg
    ...
    def __getitem__(self, indexA):
        ...
        foot_contact = self.all_foot_contact[indexA]

        quatB = self.all_quatB[indexA]
        delta_qs = self.all_dqs[indexA]
        delta_qg = self.all_dqg[indexA]
        ...
        # calc offset to get world position
        offsetA = global_i[:stidx,:-1].sum(axis=0) # 3 : rotation excluded
        ...
        foot_contact = foot_contact[stidx : (stidx + max_len)]
        if foot_contact.shape[0] < max_len:
            zeros = np.zeros((max_len - foot_contact.shape[0], 4))
            zeros[:, :, 0] = 1.0
            foot_contact = np.concatenate((foot_contact, zeros))

        # ---------------------------------- Character B ----------------------------------------------------

        quatB = quatB[stidx:stidx + max_len]
        if quatB.shape[0] < max_len:
            zeros = np.zeros((max_len - quatB.shape[0], n_joints, 4))
            zeros[:, :, 0] = 1.0
            quatB = np.concatenate((quatB, zeros))

        delta_qs = delta_qs[stidx:stidx + max_len]
        if delta_qs.shape[0] < max_len:
            zeros = np.zeros((max_len - delta_qs.shape[0], n_joints, 4))
            zeros[:, :, 0] = 1.0
            delta_qs = np.concatenate((delta_qs, zeros))

        delta_qg = delta_qg[stidx:stidx + max_len]
        if delta_qg.shape[0] < max_len:
            zeros = np.zeros((max_len - delta_qg.shape[0], n_joints, 4))
            zeros[:, :, 0] = 1.0
            delta_qg = np.concatenate((delta_qg, zeros))


        cskelB = self.train_skel_B[indexA][0:max_len]
        ...
        offsetB = offsetA / height_a * height_b
        ...
        shapeB = self.shape_dic[self.target_char].reshape(-1)
