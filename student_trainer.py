...
from src.student_model import DistilledRetNet
...
def get_parser():
    ...
    parser.add_argument("--freeze_skel", action="store_true", help="Enable skeleton module freezing")
...
def load_model(model, optimizer, resume_path, teacher_path, freeze_encoder, freeze_skel, device):
    ...
    if freeze_skel:
        print("Skeleton Module Freezed")
        for key, param in model.named_parameters():
            if 'delta_qs_dec' in key:
                param.requires_grad = False
...
def masked_l1_loss(pred, target, mask, eps=1e-6):
    ...
    diff = torch.square(pred - target) * m
...
def train_one_epoch(epoch, model, loader, optimizer, device, args):
    ...
    for it, batch in enumerate(loader):
        quat_src, skel_src, shape_src, dqs_teacher, dqg_teacher, quat_tgt, mask, q_mean, q_std = batch
        ...
        q_mean = q_mean.to(device).float()
        q_std = q_std.to(device).float()


        optimizer.zero_grad(set_to_none=True)

        pred_dqs, pred_dqg = model(quat_src, skel_src, shape_src, q_mean, q_std)
        ...
        if (it + 1) % 10 == 0:
            print(
                ...
                f"Loss {t_loss/(it+1):.6f} dqs {t_dqs/(it+1):.6f} dqg {t_dqg/(it+1):.6f}"
            )

    return t_loss / len(loader), t_dqs / len(loader), t_dqg / len(loader)
...
def main(args):
    ...
    start_epoch = 0
    best_val = float("inf")
    prev_epoch, prev_best = load_model(model, optimizer, args.resume_weights, args.teacher_weights, args.freeze_encoder, args.freeze_skel, device)
    ...
    for epoch in range(start_epoch, args.num_epoch):
        train_loss, train_dqs, train_dqg = train_one_epoch(epoch, model, train_loader, optimizer, device, args)
        val_loss, val_dqs, val_dqg = eval_one_epoch(epoch, model, val_loader, device, args, mode='Val')
        test_loss, test_dqs, test_dqg = eval_one_epoch(epoch, model, test_loader, device, args, mode='Test')

        scheduler.step(val_loss)

        history["epoch"].append(epoch)
        history["train_loss"].append(float(train_loss))
        history["train_loss_dqs"].append(float(train_dqs))
        history["train_loss_dqg"].append(float(train_dqg))
        history["val_loss"].append(float(val_loss))
        history["val_loss_dqs"].append(float(val_dqs))
        history["val_loss_dqg"].append(float(val_dqg))
        ...
        elif args.monitor == "val_loss_dqs":
            monitor_value = val_dqs
        else:
            monitor_value = val_dqg
