...
class DistilledRetNet(nn.Module):
    def __init__(
        ...
        self.delta_leftArm_dec = DeltaShapeDecoder(
            3, hidden_channels_p, kp
        )
        self.delta_rightArm_dec = DeltaShapeDecoder(
            3, hidden_channels_p, kp
        )
        self.delta_leftLeg_dec = DeltaShapeDecoder(
            2, hidden_channels_p, kp
        )
        self.delta_rightLeg_dec = DeltaShapeDecoder(
            2, hidden_channels_p, kp
        )
    ...
    def forward(
        ...
        for t in range(T):
            ...
            qoutA_t_denorm = quatA_t * quat_std + quat_mean

            # delta qs
            delta_qs_t = self.delta_qs_dec(quatA_t, skelA)
            delta_qs_t = delta_qs_t * quat_std + quat_mean
            delta_qs_t = normalized(delta_qs_t)
            delta_qs.append(delta_qs_t)

            qB_base = q_mul_q(qoutA_t_denorm, delta_qs_t)
            ...
            #delta_qg_t_denormed = delta_qg_t * quat_std + quat_mean
            #delta_qg_t_denormed = normalized(delta_qg_t_denormed)
            #delta_qg.append(delta_qg_t_denormed)
            

        # stack all frames
        delta_qs = torch.stack(delta_qs, dim=1)  # shape: (batch_size, T, 22, 4)
        delta_qg = torch.stack(delta_qg, dim=1)  # shape: (batch_size, T, 22, 4)
