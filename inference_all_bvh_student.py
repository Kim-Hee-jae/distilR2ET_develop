...
def load_from_bvh(
    ...
    inp_seq = torch.from_numpy(inp_seq.astype(np.single))[None, :].to(device)
    inpskel = torch.from_numpy(inpskel.astype(np.single))[None, :].to(device)
    tgtskel = torch.from_numpy(tgtskel.astype(np.single))[None, :].to(device)
    inp_shape = torch.from_numpy(inp_shape.astype(np.single))[None, :].to(device)
    tgt_shape = torch.from_numpy(tgt_shape.astype(np.single))[None, :].to(device)
    inpquat = torch.from_numpy(inpquat.astype(np.single))[None, :].to(device)
    inp_height_ = torch.zeros((1, 1)).to(device)
    tgt_height_ = torch.zeros((1, 1)).to(device)
...
def getmodel(weight_path, arg):
    model = DistilledRetNet(**arg.ret_model_args).to(arg.device[0])
    if arg.device[0] != 'cpu':
...
def inference(ret_model, parents, inp_shape_path, tgt_shape_path, stats_path, inp_bvh_path, tgt_bvh_path):
    ...
    (
        ...
        inpjoints,
        tgtjoints,
    ) = load_from_bvh(arg.device[0], inp_shape_path, tgt_shape_path, stats_path, inp_bvh_path, tgt_bvh_path)

    # patch: tgtskel from t-pose
    ...
    if tgtskel.shape[1] != inpskel.shape[1]:
        ...
        # tgtftime = inpftime
        # tgtskel = tgtskel[:,:1]
        # tgtanim = inpanim.copy()

    # print(tgtskel.shape, inpskel.shape)
    # print(tgtanim.shape, tgtftime)
    ...
    '''
    if str(arg.device[0]).lower() != 'cpu' and torch.cuda.is_available():
    ...
    # end = time.time()
    # print((end - start)/inp_seq.shape[1])
    ...
    localA = (
        ...
        * torch.from_numpy(local_std)[None, :].to(arg.device[0])
        + torch.from_numpy(local_mean)[None, :].to(arg.device[0])
    )
    localB = (
        localB * torch.from_numpy(local_std)[None, :].to(arg.device[0])
        + torch.from_numpy(local_mean)[None, :].to(arg.device[0])
    )
    ...
    from_name = inp_bvh_path.replace('\\', '/').split('/')[-2]
    to_name = tgt_bvh_path.replace('\\', '/').split('/')[-2]
    bvh_name = inp_bvh_path.replace('\\', '/').split('/')[-1].split('.')[0]

    # BVH.save(
    #     join(arg.save_path, from_name, bvh_name + '_inp.bvh'), inpanim, inpname, inpftime
    # )

    # BVH.save(
    #     join(arg.save_path, from_name, bvh_name + '_gt.bvh'), tgtanim, tgtname, tgtftime
    # )

    bvh_path = join(arg.save_path, from_name, bvh_name + '_r2et.bvh')

    """ Ours bvh file """
    ## PATCH
    # print(tgtanim.shape)
    # if tgtanim.shape[0] != inpanim.shape[0]:
    #     tgtanim = inpanim
    #     print("CHANGED")
...
def main(arg):
    ...
    src_char_list = listdir(arg.load_inp_data['inp_bvh_dir_path'])
    for src_char in src_char_list:
        ...
        if exists(save_dir_path):
            ...
            print(f'Folder: {save_dir_path} <- R2ET inference results must be first')
        ...
        bvh_list = [bvh_name for bvh_name in listdir(bvh_dir_path) if bvh_name.endswith('.bvh')]
        total_elapsed_ms = 0
        total_frame = 0
        for bvh_name in tqdm(bvh_list):
            inp_bvh_path = join(bvh_dir_path, bvh_name)
            tgt_bvh_path = join(arg.load_inp_data['tgt_bvh_path'], bvh_name)

            elapsed_ms, T = inference(retarget_net, parents, 
                      inp_shape_path, 
                      arg.load_inp_data['tgt_shape_path'], 
                      arg.load_inp_data['stats_path'],
                      inp_bvh_path,
                      tgt_bvh_path
                      )
        ...
        print(total_elapsed_ms / total_frame)
