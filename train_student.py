...
def train(
    ...
    for batch_idx, (
        ...
        # delta_qs_loss = DistilledRetNet.get_quat_loss(joint_lst, 22, foot_contact_mask, mask, delta_qs_gt, delta_qs).mean()
        # delta_qg_loss = DistilledRetNet.get_quat_loss(joint_lst, 22, foot_contact_mask, mask, delta_qg_gt, delta_qg).mean()
        # quat_loss = DistilledRetNet.get_quat_loss(joint_lst, 22, foot_contact_mask, mask, quatB_gt, quatB_rt).mean()

        base_loss = (
            arg.weight_dqs * delta_qs_loss + arg.weight_dqg * delta_qg_loss + arg.weight_quat * quat_loss
        )
        if include_fc:
            # foot_contact_loss = DistilledRetNet.get_foot_contact_loss(joint_lst, 22, foot_contact_mask, mask, localB_rt, localB_gt).mean()
            localB_rt_denorm = localB_rt * local_std[None, :] + local_mean[None, :]
            # localB_gt_denorm = localB_gt * local_std[None, :] + local_mean[None, :]
            ...
            # foot_contact_loss_gt_, global_joints_y_gt = DistilledRetNet.get_foot_contact_loss_(joint_lst, foot_contact_mask, mask, heightB, localB_gt_denorm, globalB_rt, offsetB)

            # for i in range(global_joints_y.shape[0]):
            #     if (global_joints_y_gt[i] < -20).sum() > 0:
            #         print(global_joints_y[i])
            #         # print(global_joints_y_gt[i])
            #         print(foot_contact_loss_[i])
            #         # print(globalB_rt[i])
            #         print(offsetB[i])
            #         # print(localB_rt_denorm[i][:,joint_lst,1])
        else:
...
def main(arg):
    ...
    for i in range(arg.epoch):
        ...
        if epoch_loss_q.avg < 0.0002:
            break
    i += 1
    for j in range(i, i + int(arg.epoch / 2)):
        epoch_loss_r, epoch_loss_q, epoch_loss_foot, epoch_time = train(
            retarget_net,
            data_loader,
            optimizer_ret,
        ...
        if (j - i + 1) % 5 == 0:
            ...
            torch.save(
                weights_gen,
                os.path.join(
                    arg.work_dir, arg.model_save_name + '_ret-' + str(j + 1) + '.pt'
                ),
            )
            log_txt = arg.model_save_name + '_ret-' + str(j + 1) + '.pt has been saved!'
            print_log_txt(log_txt, arg.work_dir)
